...
pte_osResult pte_osInit(void)
{
	...
	//flexos_nop_gate_r(0, 0, crnt, uk_thread_current);
	crnt = uk_thread_current();
	//uk_pr_crit("crnt %p\n", crnt);
	UK_ASSERT((((uintptr_t)crnt)%16) == 0);
	UK_ASSERT(((uintptr_t)crnt->prv%16) == 0);
	UK_ASSERT(__builtin_cheri_tag_get(crnt) == 1);
	//UK_ASSERT(__builtin_cheri_tag_get(crnt->prv) == 1);
	//uk_pr_crit("Perms %d\n", __builtin_cheri_perms_get(crnt->prv));

	flexos_nop_gate(0, 0, uk_thread_set_private, crnt, ptd);
	ptd->uk_thread = crnt;
...
static pte_thread_data_t *handle_to_ptd(pte_osThreadHandle h)
{
	...
	flexos_nop_gate_r(0, 0, res, uk_thread_get_private, h);
	return res;
}
...
static pte_thread_data_t *current_ptd(void)
{
	...
	flexos_nop_gate_r(0, 0, thread, uk_thread_current);
	flexos_nop_gate_r(0, 0, ptd, uk_thread_get_private, thread);
	return ptd;
}
	...
	pte_osThreadHandle *ph)
{
	...
	ptd = uk_malloc(flexos_shared_alloc, sizeof(pte_thread_data_t));
	if (!ptd)
	...
	ptd->done = 0;
	uk_pr_crit("create theread pointing to %p\n", ptd->entry_point);
	flexos_nop_gate_r(0, 0, ptd->uk_thread, uk_thread_create_attr, NULL,
...
#if CONFIG_LIBUKSIGNAL
	/* inherit signal mask */
	flexos_nop_gate(0, 0, uk_thread_inherit_signal_mask, ptd->uk_thread);
#endif

	flexos_nop_gate(0, 0, uk_thread_set_private, ptd->uk_thread, ptd);
...
{
	if (h->sched)
		flexos_nop_gate(0, 0, uk_thread_kill, h);
...
void pte_osThreadExit(void)
{
	...
	ptd->done = 1;
	flexos_nop_gate(0, 0, uk_sched_thread_exit);
}
...
pte_osResult pte_osThreadWaitForEnd(pte_osThreadHandle h)
{
	...
	//uk_pr_crit("pte wait, ptd id: %d, current: %d\n", ptd->uk_thread->tid, self_ptd->uk_thread->tid);
	while (1) {
	//	uk_pr_crit("Whiling away\n");
		if (is_exited(ptd->uk_thread)) {
			flexos_nop_gate(0, 0, uk_thread_wait, ptd->uk_thread);
			return PTE_OS_OK;
		}
...
pte_osThreadHandle pte_osThreadGetHandle(void)
{
	...
	flexos_nop_gate_r(0, 0, cur, uk_thread_current);
	return cur;
}
...
int pte_osThreadGetPriority(pte_osThreadHandle h)
{
	...
	flexos_nop_gate_r(0, 0, ret, uk_thread_get_prio, ptd->uk_thread,
			  &prio);
...
pte_osResult pte_osThreadSetPriority(pte_osThreadHandle h, int new_prio)
{
	...
	flexos_nop_gate_r(0, 0, ret, uk_thread_set_prio, ptd->uk_thread,
			  new_prio);
...
void pte_osThreadSleep(unsigned int msecs)
{
	...
	flexos_nop_gate(0, 0, uk_sched_thread_sleep, nsec);
}
...
pte_osResult pte_osMutexCreate(pte_osMutexHandle *ph)
{
	...
	m = uk_malloc(flexos_shared_alloc, sizeof(struct uk_mutex));
	if (!m)
		return PTE_OS_NO_RESOURCES;
...
pte_osResult pte_osSemaphoreCreate(int init_value, pte_osSemaphoreHandle *ph)
{
	...
	s = uk_malloc(flexos_shared_alloc, sizeof(struct uk_semaphore));
	if (!s)
		return PTE_OS_NO_RESOURCES;
