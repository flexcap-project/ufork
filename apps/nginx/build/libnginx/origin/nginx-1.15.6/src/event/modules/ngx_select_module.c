...
static fd_set         __uk_tls master_read_fd_set;
static fd_set         __uk_tls master_write_fd_set;
static fd_set         __uk_tls work_read_fd_set;
static fd_set         __uk_tls work_write_fd_set;

static ngx_int_t     __uk_tls max_fd;
static ngx_uint_t    __uk_tls nevents;

static ngx_event_t  __uk_tls **event_index;
    ...
    ngx_uint_t flags)
{
 //   while(1);
    ...
    struct timeval     *tp;
    ngx_connection_t  *c;
    uk_pr_info("ngx_select_process_events\n");
   // sfork_syscalls.printf("select LR %p\n", LR());
    ...
    uk_pr_info("ngx_select_process_events1\n");
    ...
    uk_pr_info("ngx_select_process_events2, master read set %p\n", &master_read_fd_set);

    flexos_gate_r(liblwip, ready, select, max_fd + 1, &work_read_fd_set, &work_write_fd_set, NULL, tp);
//uk_pr_crit("ngx_select_process_events3, pid %d\n", PROCESS_ID());
    err = (ready == -1) ? ngx_errno : 0;

    if (flags & NGX_UPDATE_TIME || ngx_event_timer_alarm) {
        ngx_time_update();
    }
    uk_pr_info("ngx_select_process_events4\n");
...
uk_pr_info("ngx_select_process_events5\n");
        ngx_log_error(level, cycle->log, err, "select() failed");
        uk_pr_info("ngx_select_process_events6\n");

        if (err == NGX_EBADF) {
            uk_pr_info("ngx_select_process_events7\n");
            ngx_select_repair_fd_sets(cycle);
            uk_pr_info("ngx_select_process_events8\n");
        }
       // sfork_syscalls.printf("Uh oh error 1\n");
        return NGX_ERROR;
    }
    ...
    if (ready == 0) {
        ...
        // }
        return NGX_OK;
uk_pr_info("ngx_select_process_events9\n");
        ngx_log_error(NGX_LOG_ALERT, cycle->log, 0,
                      "select() returned no events without timeout");
                     // sfork_syscalls.printf("Uh oh error 2, %d\n", PROCESS_ID());
        return NGX_ERROR;
    }
    ...
    for (i = 0; i < nevents; i++) {
        ...
        if (found) {
            ...
            queue = ev->accept ? &ngx_posted_accept_events
                               : &ngx_posted_events;
uk_pr_info("ngx_select_process_events9.1\n");
            ngx_post_event(ev, queue);
            uk_pr_info("ngx_select_process_events9.2\n");
    ...
    if (ready != nready) {
        uk_pr_info("ngx_select_process_events10\n");
        ngx_log_error(NGX_LOG_ALERT, cycle->log, 0,
                      "select ready != events: %d:%d", ready, nready);

        ngx_select_repair_fd_sets(cycle);
        uk_pr_info("ngx_select_process_events11\n");
    }
