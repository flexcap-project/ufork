...
main(int argc, char *const *argv)
{
    ...
    uk_pr_info("Did process options\n");

    if (ngx_os_init(log) != NGX_OK) {
        return 1;
    }

    uk_pr_info("Did os init\n");
    ...
    uk_pr_info("Did crc32 table init\n");
    /*
    ...
    uk_pr_info("Did slab sizes init\n");
 
    if (ngx_add_inherited_sockets(&init_cycle) != NGX_OK) {
        return 1;
    }

    uk_pr_info("Did inherit sockets\n");

    if (ngx_preinit_modules() != NGX_OK) {
        return 1;
    }

    uk_pr_info("Did preinit modules\n");
    ...
    uk_pr_info("Did test config stuff\n");
...
ngx_core_module_init_conf(ngx_cycle_t *cycle, void *conf)
{
    ...
    //uk_pr_info("core module init 2\n");
#if !(NGX_WIN32)

    if (ccf->user == (uid_t) NGX_CONF_UNSET_UINT && geteuid() == 0) {
        //uk_pr_info("core module init 2.1\n");
        struct group   *grp;
        struct passwd  *pwd;

        ngx_set_errno(0);
        //uk_pr_info("core module init 2.2\n");
        pwd = getpwnam(NGX_USER);
        //uk_pr_info("core module init 2.3\n");
        if (pwd == NULL) {
            ngx_log_error(NGX_LOG_EMERG, cycle->log, ngx_errno,
                          "getpwnam(\"" NGX_USER "\") failed");
            return NGX_CONF_ERROR;
        }
//uk_pr_info("core module init 2.4\n");
        ccf->username = NGX_USER;
        //uk_pr_info("core module init 2.4.1 %d\n", ccf->user);
        //ccf->user = pwd->pw_uid;
        ccf->user = 0;
//uk_pr_info("core module init 2.5\n");
        ngx_set_errno(0);
        //uk_pr_info("core module init 2.6\n");
        grp = getgrnam(NGX_GROUP);
        //uk_pr_info("core module init 2.7\n");
        if (grp == NULL) {
            ngx_log_error(NGX_LOG_EMERG, cycle->log, ngx_errno,
                          "getgrnam(\"" NGX_GROUP "\") failed");
            return NGX_CONF_ERROR;
        }
//uk_pr_info("core module init 2.8\n");
        ccf->group = grp->gr_gid;
    }

//uk_pr_info("core module init 3\n");
