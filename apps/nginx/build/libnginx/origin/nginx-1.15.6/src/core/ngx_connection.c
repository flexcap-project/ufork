             ...
             const void *optval, socklen_t optlen)
{
	...
	ret = setsockopt(sockfd, level, optname, _optval, optlen);
	return ret;
}
...
static inline int getsockopt_wrapper(int sockfd, int level, int optname, void *optval, socklen_t *optlen)
{
	...
	char _optval[*optlen];
	memcpy(_optval, optval, *optlen);
	ret = getsockopt(sockfd, level, optname, _optval, _optlen);
	*optlen = *_optlen;
	return ret;
}
    ...
    for (tries = 5; tries; tries--) {
        ...
        for (i = 0; i < cycle->listening.nelts; i++) {
            ...
            if (ls[i].add_reuseport) {
                ...
                int  *reuseport = malloc(sizeof(int));
                *reuseport = 1;
		...
		free(reuseport);
            ...
            if (ls[i].reuseport && !ngx_test_config) {
                int  *_reuseport = malloc(sizeof(int));
...
#undef reuseport
            ...
            if (ls[i].sockaddr->sa_family == AF_INET6) {
                int  *ipv6only = malloc(sizeof(int));
...
uk_pr_info("ngx get connection 1, socket fd %d\n", s);
...
__asm__ ("nop\n nop\n nop\n");
uk_pr_info("ngx_xycle %p, c22 %p, tls %p\n", ngx_cycle, C_REG(), PROCESS_LOCAL_STORAGE());
    c = ngx_cycle->free_connections;
    __asm__ ("nop\n nop\n nop\n nop\n");
    uk_pr_info("Number of connections %d, free %d, reusable %d, cycle hostname %p, %p, lr %p, sp %p\n", ngx_cycle->connection_n, ngx_cycle->free_connection_n, ngx_cycle->reusable_connections_n, ngx_cycle, &ngx_cycle, LR(), SP());

    if (c == NULL) {
        uk_pr_info("ngx get connection 1.5\n");
        ngx_drain_connections((ngx_cycle_t *) ngx_cycle);
        c = ngx_cycle->free_connections;
    }

    if (c == NULL) {
        uk_pr_info("ngx get connection 2\n");
        ngx_log_error(NGX_LOG_ALERT, log, 0,
    ...
    uk_pr_info("Number of free connections reduced\n");
...
ngx_close_connection(ngx_connection_t *c)
{
    ...
    ngx_socket_t  fd;
uk_pr_info("Ngx close connection lr %p\n", LR());
