...
ngx_create_pool_shared(size_t size, ngx_log_t *log)
{
    ...
    //uk_pr_info("Pre memalign %d\n", size);
    p = ngx_memalign(NGX_POOL_ALIGNMENT, size, log);
    if (p == NULL) {
        return NULL;
    }

    //uk_pr_info("Post memalign %d\n", size);
...
#if !(NGX_DEBUG_PALLOC)
    if (size <= pool->max) {
        return ngx_palloc_small(pool, size, 16);
    }
#endif
...
ngx_palloc_large_shared(ngx_pool_t *pool, size_t size)
{
    ...
    p = malloc(size);
    uk_pr_info("Malloc large shared %p, pid %d\n", p, PROCESS_ID());
    if (p == NULL) {
        return NULL;
    }
    ...
    large = ngx_palloc_small(pool, sizeof(ngx_pool_large_t), 16);
    if (large == NULL) {
        free(p);
        return NULL;
    }
...
ngx_palloc_large(ngx_pool_t *pool, size_t size)
{
    ...
    ngx_uint_t         n;
    ngx_pool_large_t  *large;
    //uk_pr_info("Malloc size %d\n", size);
    p = ngx_alloc(size, pool->log);
    uk_pr_info("Malloc pool large %p, pid %d\n", p, PROCESS_ID());
    if (p == NULL) {
        return NULL;
    }
...
ngx_pcalloc(ngx_pool_t *pool, size_t size)
{
    ...
    uk_pr_info("Malloc pcalloc %p, pid %d\n", p, PROCESS_ID());
    if (p) {
        ngx_memzero(p, size);
    }
...
ngx_pcalloc_shared(ngx_pool_t *pool, size_t size)
{
    ...
    uk_pr_info("Malloc pcalloc shared %p, pid %d\n", p, PROCESS_ID());
    if (p) {
        ngx_memzero(p, size);
    }
