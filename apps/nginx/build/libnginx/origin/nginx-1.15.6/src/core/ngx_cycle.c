...
volatile __uk_tls ngx_cycle_t  *ngx_cycle;
...
ngx_init_cycle(ngx_cycle_t *old_cycle)
{
    ...
    //uk_pr_info("Pre create pool\n");
    pool = ngx_create_pool(NGX_CYCLE_POOL_SIZE, log);
    if (pool == NULL) {
        return NULL;
    }

    //uk_pr_info("Post create pool\n");
    pool->log = log;

    /* using the same log as pool, hopefully this is fine... */
    //uk_pr_info("Pre create pool 1\n");
...
//uk_pr_info("cycle init 9\n");

    cycle->conf_ctx = ngx_pcalloc(pool, ngx_max_module * sizeof(void *));
    //uk_pr_info("cycle init 9.1, %p\n", cycle->conf_ctx);
    if (cycle->conf_ctx == NULL) {
        ngx_destroy_pool(pool);
        ngx_destroy_pool_shared(pool_shared);
        return NULL;
    }
    //uk_pr_info("cycle init 10\n");
    ...
    //uk_pr_info("cycle init 12\n");

    ngx_strlow(cycle->hostname.data, (u_char *) hostname, cycle->hostname.len);
    //uk_pr_info("cycle init 13\n");
    ...
    //uk_pr_info("cycle init 14\n");


    for (i = 0; cycle->modules[i]; i++) {
        //uk_pr_info("cycle modules 1\n");
        if (cycle->modules[i]->type != NGX_CORE_MODULE) {
            continue;
        }
        //uk_pr_info("cycle modules 2\n");

        module = cycle->modules[i]->ctx;
        //uk_pr_info("cycle modules 3, create conf %p, index %d\n", module->create_conf, cycle->modules[i]->index);
        if (module->create_conf) {
            //uk_pr_info("cycle modules 4\n");
            rv = module->create_conf(cycle);
            //uk_pr_info("cycle modules 5\n");
            if (rv == NULL) {
                ngx_destroy_pool(pool);
        	ngx_destroy_pool_shared(pool_shared);
                return NULL;
            }
            //uk_pr_info("cycle modules 6, %p, %d\n", cycle->conf_ctx, cycle->modules[i]->index);
            cycle->conf_ctx[cycle->modules[i]->index] = rv;
            //uk_pr_info("cycle modules 6.1\n");
        }
        //uk_pr_info("cycle modules 7\n");
    }
//uk_pr_info("cycle init 15\n");
...
uk_pr_info("cycle init 18\n");
    ...
    //uk_pr_info("cycle init 19\n");

    if (ngx_test_config && !ngx_quiet_mode) {
        ngx_log_stderr(0, "the configuration file %s syntax is ok",
                       cycle->conf_file.data);
    }
    //uk_pr_info("cycle init 19.1\n");

    for (i = 0; cycle->modules[i]; i++) {
        //uk_pr_info("per module 1\n");
        if (cycle->modules[i]->type != NGX_CORE_MODULE) {
            continue;
        }
        //uk_pr_info("per module 2\n");

        module = cycle->modules[i]->ctx;
        //uk_pr_info("per module 3, module->init_conf %p\n", module->init_conf);

        if (module->init_conf) {
            //uk_pr_info("per module 4, init_conf %p, cycle %p, conf_ctx %p, conf %p, modules[i]->index %d, the module %p\n", module->init_conf, cycle, cycle->conf_ctx, cycle->conf_ctx[cycle->modules[i]->index], cycle->modules[i]->index, ngx_modules[cycle->modules[i]->index]);
            if (module->init_conf(cycle,
                                  cycle->conf_ctx[cycle->modules[i]->index])
                == NGX_CONF_ERROR)
            {
                //uk_pr_info("per module 5\n");
                environ = senv;
                ngx_destroy_cycle_pools(&conf);
                return NULL;
            }
            //uk_pr_info("per module 6\n");
        }
    }
    uk_pr_info("cycle init 19\n");
