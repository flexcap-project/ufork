...
ngx_http_init_connection(ngx_connection_t *c)
{
    ...
    uk_pr_info("Http init connection pool %p, hc %p, c%p\n", c->pool, hc, c);
    if (hc == NULL) {
    ...
    if (rev->ready) {
        ...
        if (ngx_use_accept_mutex) {
            uk_pr_info("Post event init connection rev %p, pid %d\n", rev, PROCESS_ID());
...
ngx_http_wait_request_handler(ngx_event_t *rev)
{
    ...
    ngx_http_core_srv_conf_t  *cscf;
     ...
     uk_pr_info("http wait request handler , lr %p\n", LR());
    ...
    uk_pr_info("http wait request handler 1, c %p , tag %d, c->recv %p, b %p, b->last %p\n", c, __builtin_cheri_tag_get(c), c->recv, b, b->last);

    n = c->recv(c, b->last, size);

    uk_pr_info("http wait request handler 1.1 \n");

    if (n == NGX_AGAIN) {
        uk_pr_info("http wait request handler 1.2 \n");
        if (!rev->timer_set) {
            ngx_add_timer(rev, c->listening->post_accept_timeout);
            ngx_reusable_connection(c, 1);
        }
        uk_pr_info("http wait request handler 1.3 \n");

        if (ngx_handle_read_event(rev, 0) != NGX_OK) {
            ngx_http_close_connection(c);
            return;
        }
        uk_pr_info("http wait request handler 1.4 \n");
        ...
        uk_pr_info("http wait request handler 1.5 \n");

        return;
    }

    uk_pr_info("http wait request handler 2 \n");
    ...
    uk_pr_info("http wait request handler 3 \n");
    ...
    if (hc->proxy_protocol) {
        ...
        if (b->pos == b->last) {
            ...
            b->pos = b->start;
            b->last = b->start;
            uk_pr_info("Post event wait rev %p, pid %d\n", rev, PROCESS_ID());
            ngx_post_event(rev, &ngx_posted_events);
            return;
        }
    }
    uk_pr_info("http wait request handler 4 \n");
    ...
    uk_pr_info("http wait request handler 5\n");
...
uk_pr_info("http process req line lr %p\n", LR());
...
uk_pr_info("http process req headers lr %p\n", LR());
            ngx_http_process_request(r);
...
ngx_http_finalize_request(ngx_http_request_t *r, ngx_int_t rc)
{
    ...
    c = r->connection;
    uk_pr_info("Finalize request lr %p, PID %d\n", LR(), PROCESS_ID());
...
ngx_http_finalize_connection(ngx_http_request_t *r)
{
    ...
    {
        uk_pr_info("Keep alive r %p, pid %d\n", r, PROCESS_ID());
...
ngx_http_set_keepalive(ngx_http_request_t *r)
{
    ...
    if (b->pos < b->last) {
        ...
        rev->handler = ngx_http_process_request_line;
        uk_pr_info("Post event rev %p, pid %d\n", rev, PROCESS_ID());
        ngx_post_event(rev, &ngx_posted_events);
        return;
    }
    ...
    if (rev->ready) {
        uk_pr_info("Post event 1 rev %p, pid %d\n", rev, PROCESS_ID());
        ngx_post_event(rev, &ngx_posted_events);
    }
}
...
ngx_http_keepalive_handler(ngx_event_t *rev)
{
    ...
    uk_pr_info("http keepalive handler lr %p, rev %p, c %p\n", LR(), rev, rev->data);
    ...
    uk_pr_info("http keepalive handler 1 lr %p\n", LR());
...
#endif
uk_pr_info("http keepalive handler 2 lr %p\n", LR());
    b = c->buffer;
    size = b->end - b->start;
...
uk_pr_info("http keepalive handler 3 lr %p\n", LR());
    /*
...
uk_pr_info("http keepalive handler 4 lr %p\n", LR());
    n = c->recv(c, b->last, size);
    c->log_error = NGX_ERROR_INFO;
    ...
    uk_pr_info("http keepalive handler 5 lr %p\n", LR());

    if (n == NGX_ERROR) {
        uk_pr_info("http keepalive handler 6 lr %p\n", LR());
        ngx_http_close_connection(c);
        return;
    }
    ...
    rev->handler = ngx_http_process_request_line;
    uk_pr_info("http keep alive handler \n");
    ngx_http_process_request_line(rev);
}
