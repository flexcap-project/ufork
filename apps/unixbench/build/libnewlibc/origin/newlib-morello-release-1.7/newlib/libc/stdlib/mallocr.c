...
#define MALLOC_COPY(dest,src,nbytes) memmove(dest,src,nbytes)
#else
...
#define cALLOc_c    _calloc_r
#define fREe_c      _free_r
#define mALLOc_c    _malloc_r
#define mALLOc_raw  _malloc_raw
#define mEMALIGn_c  _memalign_r
#define rEALLOc_c   _realloc_r
#define vALLOc_c    _valloc_r
#define pvALLOc_c   _pvalloc_r
#else // !__CHERI_PURE_CAPABILITY__
#define cALLOc      _calloc_r
#define fREe        _free_r
#define mALLOc      _malloc_r
#define mALLOc_raw  _malloc_r
...
#define MINSIZE_ALIGN (1 << (8 * sizeof(int) - __builtin_clz(sizeof(struct malloc_chunk) - 1)))
...
// Capabilities are using floating point like exponent, meaning
// the size is not always representable. This rounds the size parameter
// to a representable value. A downside is this makes the object
// slightly larger than it is...
static size_t getConcentrateAlignment(size_t size) {
  size_t mask = __builtin_cheri_representable_alignment_mask(size);
  return 1UL << __builtin_ctzl(mask);
}

#endif /* __CHERI_PURE_CAPABILITY__ */

static size_t roundCapabilitySizeUpward(size_t size) {
#ifdef __CHERI_PURE_CAPABILITY__
  if (size == 0)
    return size;
  size_t old_size = size;
  size = __builtin_cheri_round_representable_length(size);
  // Check for overflow. If overflow occured, return the old size.
  if (size < old_size)
    return old_size;
  return size;
#else
...
static void roundSizeForConcentrate(size_t *size, size_t *align) {
#if defined( __CHERI_PURE_CAPABILITY__)
  *size = roundCapabilitySizeUpward(*size);
  *align = getConcentrateAlignment(*size);
#else
  *align = 1;
#endif
}
...
#define LOG2_MALLOC_ALIGNMENT 4
#else
#error MALLOC_ALIGNMENT must be 16 for pure capability mode
#endif

static uint64_t to_ptr(void* ptr, void* space)
{
#ifdef __CHERI_PURE_CAPABILITY__
    uint64_t address;
    // eventually this needs to be a single instruction
    asm volatile("gcvalue  x0, %1;"     // get absolute address
                 "gcbase x1, %2;"       // base of new space
                 "sub    %0, x0, x1;"   // offset in new space
                 : "=r"(address) : "C"(ptr), "C"(space) : "x0","x1" );
    return address;
#else
    return (uint64_t)ptr;
#endif
...
// convert a user capability with bounds around the user requested memory
// into a heap capability with heap bounds
static Void_t* mem2heap(capability Void_t* mem)
{
#ifdef __CHERI_PURE_CAPABILITY__
    ...
    if (mem) {
       ...
       // convert to a heap offset.
       size_t heap_offset = to_ptr(mem, sbrk_base);

       // and then to a heap pointer
       hmem = cheri_offset_set(sbrk_base, heap_offset);

    }

    // same address, different bounds
    assert(hmem == mem);

    return hmem;
#else // __CHERI_PURE_CAPABILITY__
    return (__cheri_fromcap Void_t *)mem;
#endif // __CHERI_PURE_CAPABILITY__
}

// convert a heap capability to a bounded user capability
static capability Void_t* heap2mem(Void_t* hmem, size_t bytes)
{
#ifdef __CHERI_PURE_CAPABILITY__
    Void_t* m = hmem;
#else
    capability Void_t* m = (capability Void_t*)hmem;
#endif

    if (m != 0)
    {
        m = __builtin_cheri_bounds_set_exact(m, bytes);
        assert(cheri_tag_get(m) == 1);
    }
...
capability Void_t* cALLOc_c(RARG size_t n, size_t elem_size)
{
    size_t bytes = roundCapabilitySizeUpward(n * elem_size);
    return heap2mem(cALLOc(RCALL n, elem_size), bytes);
}

void fREe_c(RARG capability Void_t* mem)
{
    return fREe(RCALL mem2heap(mem));
}

capability Void_t* mALLOc_c(RARG size_t bytes)
{
    bytes = roundCapabilitySizeUpward(bytes);
    return heap2mem(mALLOc(RCALL bytes), bytes);
}

capability Void_t* rEALLOc_c(RARG capability Void_t* mem, size_t bytes)
{
    bytes = roundCapabilitySizeUpward(bytes);
    return heap2mem(rEALLOc(RCALL mem2heap(mem), bytes), bytes);
}

capability Void_t* mEMALIGn_c(RARG size_t alignment, size_t bytes)
{
    size_t req_align;
    roundSizeForConcentrate(&bytes, &req_align);
    if (alignment < req_align)
      alignment = req_align;
    return heap2mem(mEMALIGn(RCALL alignment, bytes), bytes);
}

#ifdef DEFINE_VALLOC
capability Void_t* vALLOc_c(RARG size_t bytes)
{
    bytes = roundCapabilitySizeUpward(bytes);
    return heap2mem(vALLOc(RCALL bytes), bytes);
}
#endif // DEFINE_VALLOC

#ifdef DEFINE_PVALLOC
capability Void_t* pvALLOc_c(RARG size_t bytes)
{
    bytes = roundCapabilitySizeUpward(bytes);
    return heap2mem(pvALLOc(RCALL bytes), bytes);
}
...
#else
  size_t align;
  roundSizeForConcentrate(&bytes, &align);
  return mEMALIGn(RCALL align, bytes);
#endif
}
...
#if __STD_C
Void_t* mALLOc_raw(RARG size_t bytes)
#else
Void_t* mALLOc_raw(RARG bytes) RDECL size_t bytes;
#endif
{
#ifdef MALLOC_PROVIDED
...
#endif
{
  ...
  int reuse_memory;              /* can reuse memory */
...
#if defined(__CHERI_PURE_CAPABILITY__)
  /* We can easily reuse memory if we don't have additional
     alignment requirements. */
  reuse_memory = (getConcentrateAlignment(nb) <= MALLOC_ALIGNMENT);
#else
  reuse_memory = 1;
#endif
  ...
  if ((long)(oldsize) < (long)(nb))  
  {
    if (!reuse_memory)
    {
      /* The realloc has alignment requirements, just allocate. */
      newmem = mALLOc (RCALL bytes);
...
#endif
{
  ...
  if (alignment <= MALLOC_ALIGNMENT) return mALLOc_raw(RCALL bytes);

  /* Otherwise, ensure that it is at least a minimum chunk size */
  
  if (alignment <  MINSIZE) alignment = MINSIZE_ALIGN;
  ...
  m  = (char*)(mALLOc_raw(RCALL nb + alignment + MINSIZE));
...
#endif
{
  ...
  INTERNAL_SIZE_T sz = roundCapabilitySizeUpward(n * elem_size);
