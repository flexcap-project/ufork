...
int ukplat_coutk_kernel(const char *buf __maybe_unused, unsigned int len)
{
	for (unsigned int i = 0; i < len; i++) {
...
int ukplat_coutk(const char *buf __maybe_unused, unsigned int len)__attribute__((section(".app_libs_text"))){
	if (sfork_syscalls.coutk)
		return sfork_syscalls.coutk(buf, len);
	else 
		return ukplat_coutk_kernel(buf, len);
}
