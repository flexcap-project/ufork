...
static void virtio_netdev_xmit_free(struct uk_netdev_tx_queue *txq)
{
//	uk_pr_err("virtionet netdev xmit free\n");
	struct uk_netbuf *pkt = NULL;
	int cnt = 0;
	int rc;

	UK_ASSERT(__builtin_cheri_tag_get(txq) == 1);
	UK_ASSERT(__builtin_cheri_tag_get(txq->vq) == 1);
...
static int virtio_netdev_rx_fillup(struct uk_netdev_rx_queue *rxq,
				   ...
				   int notify)
{
	//uk_pr_err("virtionet netdev rx fillup, tid %d\n", uk_thread_current()->tid);
...
out:
	 ...
	 */
//	uk_pr_err("I guess it's not all bad\n");
	if (notify && filled)
...
static int virtio_netdev_xmit(struct uk_netdev *dev,
			      ...
			      struct uk_netbuf *pkt)
{
//	uk_pr_err("virtionet netdev xmit\n");
...
static int virtio_netdev_rxq_dequeue(struct uk_netdev_rx_queue *rxq,
				     struct uk_netbuf **netbuf)
{
	//uk_pr_err("virtionet rxq_dequeue\n");
	int ret;
	int rc = 0;
	...
	if (ret < 0) {
		//uk_pr_err("No data available in the queue\n");
		*netbuf = NULL;
		return rxq->nb_desc;
	}
...
static int virtio_netdev_recv(struct uk_netdev *dev,
			      ...
			      struct uk_netbuf **pkt)
{
//	uk_pr_err("virtionet netdev_recv\n");
	int status = 0x0;
	int rc = 0;
...
static struct uk_netdev_rx_queue *virtio_netdev_rx_queue_setup(
				...
				struct uk_netdev_rxqueue_conf *conf)
{
//	uk_pr_err("virtionet uk_netdev_rx_queue\n");
...
static int virtio_netdev_vqueue_setup(struct virtio_net_device *vndev,
		...
		struct uk_alloc *a)
{
//	uk_pr_err("virtionet netdev vqueue_setup\n");
	int rc = 0;
	int id = 0;
...
{
//	uk_pr_err("virtionet netdev tx_queue\n");
...
static int virtio_netdev_rxq_info_get(struct uk_netdev *dev,
				      ...
				      struct uk_netdev_queue_info *qinfo)
{
//	uk_pr_err("virtionet rxq info get\n");
	struct virtio_net_device *vndev;
...
static int virtio_netdev_txq_info_get(struct uk_netdev *dev,
				      ...
				      struct uk_netdev_queue_info *qinfo)
{
//	uk_pr_err("virtionet txq info get\n");
	struct virtio_net_device *vndev;
...
static unsigned virtio_net_promisc_get(struct uk_netdev *n)
{
	uk_pr_err("virtionet net promisc get\n");
...
static int virtio_netdev_feature_negotiate(struct virtio_net_device *vndev)
{
	uk_pr_err("virtionet feature negotiate\n");
...
static int virtio_netdev_rxtx_alloc(struct virtio_net_device *vndev,
				    const struct uk_netdev_conf *conf)
{
	uk_pr_err("virtionet rxtx alloc\n");
	int rc = 0;
	int i = 0;
	int vq_avail = 0;
	 ...
	 */
	uk_pr_err("virtionet rxtx alloc 1\n");
	...
	uk_pr_err("virtionet rxtx alloc 2\n");
	...
	uk_pr_err("virtionet rxtx alloc 3\n");
	/**
	...
	uk_pr_err("virtionet rxtx alloc 4\n");
exit:
	return rc;
...
static int virtio_netdev_configure(struct uk_netdev *n,
				   const struct uk_netdev_conf *conf)
{
	uk_pr_err("virtionet netdev configure\n");
	int rc = 0;
	struct virtio_net_device *vndev;
	...
	uk_pr_err("Configured: features=0x%lx max_virtqueue_pairs=%"__PRIu16"\n",
...
static int virtio_net_rx_intr_enable(struct uk_netdev *n,
				     struct uk_netdev_rx_queue *queue)
{
	uk_pr_err("virtionet rx intr enable\n");
...
{
	struct virtio_net_device *vndev __unused;
	uk_pr_err("virtionet rx intr disable\n");
	UK_ASSERT(n);
	vndev = to_virtionetdev(n);
...
static void virtio_net_info_get(struct uk_netdev *dev,
				struct uk_netdev_info *dev_info)
{
	uk_pr_err("virtionet net info get\n");
	struct virtio_net_device *vndev;
...
static int virtio_net_add_dev(struct virtio_dev *vdev)
{
	struct virtio_net_device *vndev;
	int rc = 0;
	uk_pr_err("virtionet net add dev\n");
	...
	vndev->hw_addr = calloc(1, sizeof(*(vndev->hw_addr)));
	...
	vndev->netdev.scratch_pad = calloc(1, __PAGE_SIZE);
